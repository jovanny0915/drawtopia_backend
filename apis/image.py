...
class OverlayCoverTitleRequest(BaseModel):
    ...
    subtitle: Optional[str] = "A Read-Aloud Adventure"
...
async def overlay_cover_title_endpoint(request: Request, body: OverlayCoverTitleRequest):
    ...
    try:
        ...
        # Subtitle below main title (reference: DM Sans 700, #F3E5CB, stroke #1C596F, shadow)
        subtitle_text = (body.subtitle or "").strip() if body.subtitle else None
        subtitle_font = None
        subtitle_x = subtitle_y = 0
        stroke_subtitle = max(2, int(10 * width / 2137))
        ...
        if subtitle_text:
            # 2× reference scale (was 100px at 2137px width)
            subtitle_font_size = max(28, min(240, int(200 * width / 2137)))
            ...
            stroke_subtitle = max(3, min(stroke_width * 3, int(20 * width / 2137)))
